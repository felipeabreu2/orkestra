...
### 🟢 Rápidos (alto valor × baixo esforço)
| # | Recurso | Nota |
|---|---|---|
| R1 | **Abrir no IDE externo** (VS Code/JetBrains) — botão que abre a pasta/arquivo no editor | resolve a edição sem G1; `shell.openExternal`/`code <path>` |
| R2 | **`orq ask --raw`** — envia bytes brutos p/ controlar TUIs/pagers | extensão do orq/AgentBus |
| R3 | **`orq ask --batch`** — pergunta a vários agentes conectados de uma vez | extensão do orq |
| R4 | **Add-menu no canvas** — botão direito no vazio → criar nó (terminal/nota/portal/arquivos) no cursor | UX; reusa o modal/store |
| R5 | **Estilo de conexão "circuito"** — trilhos ortogonais (90°) além do bezier atual | opção no TypedEdge |
| R6 | **Remover todas as conexões de um nó** — via popover/palette | pequeno (removeEdge em lote) |
| R7 | **Temas de terminal** — esquemas de cor do xterm (claro/escuro/importar) | xterm theme API |
| R8 | **User-agent por portal** — trocar o UA (mobile/desktop) p/ testar responsivo | webview.setUserAgent |
| R9 | **Lista "precisa de atenção"** — painel/contagem dos terminais ociosos (além do Shift+A) | extensão da Fase 20 |
| R10 | **Árvore: ordenar + miniaturas** — ordenar por nome/tipo/data + thumbnails de imagem/PDF | refinamento do FileTreeNode |
